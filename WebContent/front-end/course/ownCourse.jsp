...
<c:forEach var="courseVO" items="${list}">
                 ...
                 <% if(divColor == 7){divColor = 0;} %>
                   ...
                   <div class="row rowbox courseText">
                    ...
                    <hr>
		                    ...
		                    <input class="btn courseLightBtn CourseCancel" type="button" value="返回">
    ...
    <section class="ftco-section">
      ...
      <div class="container">
        ...
        <button class="noback" onclick="location.href='<%=request.getContextPath()%>/front-end/course/index_1.jsp'">
        	
        	<img src="<%=request.getContextPath()%>/front-end/course/images/exit.png" class="exit">
        </button>
          <div class="col-md-8 text-center heading-section ftco-animate">
          
            <h2 class="mb-4"><span>我</span> 的課程</h2>
            <p>快來刷卡花大錢上課 讓我們能夠賺大錢整天吃飽睡睡飽吃快來刷卡睡睡飽吃快來刷卡花大錢上課 讓我們能夠賺大錢整天吃飽睡睡賺大錢整天吃飽睡睡飽吃</p>
          </div>
          <div>
          	<img src="<%=request.getContextPath()%>/front-end/course/images/open-book.png" class="logo" id="addCourse">
          	<form action="/courselist/courselist.do">
...
<c:if test="${size == 0}">
	<div class="col-lg-12 notCourse">
		您尚未選過課程
		<img src="<%=request.getContextPath()%>/front-end/course/images/empty-cart.png" style="height:150px;width:150px;">
	</div>
</c:if>        
...
<c:if test="${size > 0}">
        <div class="row col-lg-12">
			            ...
			            <div class="col-lg-12 col-md-10 CourseTitle">
				              ...
				              <div class="courseTitle col-lg-1 col-md-1">oooo</div>
			              </div>
			 
	  </div> 
		<% courseColor=0; divColor = 0;%>
	<c:forEach var="courseVO" items="${coulist}">
		<c:if test="${courseVO.cou_status != 0}">
		
		
		
		<%-- ===========================課程資訊=============================== --%>
	<div class="row col-lg-12">
		 <div class="col-lg-12 col-md-10 myCourse" style="background:<%=bgColor.get(divColor++) %> !important">
			              ...
			              <c:if test="${courseVO.cou_status == 1}">
			              		<div class="showCoures col-lg-1 col-md-1" style="color:red;">${status[courseVO.cou_status]} </div>
			              </c:if>
			              <c:if test="${courseVO.cou_status == 2}">
			              		<div class="showCoures col-lg-1 col-md-1" style="color:green;">${status[courseVO.cou_status]} </div>
			              </c:if>
			              <c:if test="${courseVO.cou_status == 3}">
			              		<div class="showCoures col-lg-1 col-md-1" style="border:1px solid">${status[courseVO.cou_status]} </div>
			              </c:if>
			             
		          <div class="showCoures col-lg-1 col-md-1 magnifier1">
			                <img src="<%=request.getContextPath()%>/front-end/course/images/magnifier.png" class="magnifier">
			                <input type="hidden" value="${courseVO.cou_num}">
			      </div>
			      <c:set var="i" value="0"/>
			      <c:if test="${courseVO.cou_status == 1}">
			      	<c:forEach var="coul" items="${coulSvc.getCou_NumBySt_Num(stp.st_num)}">
						<c:if test="${(coul.cou_num == courseVO.cou_num) && (coul.pay_status == 0)}">	
				      		<form METHOD="post" ACTION="<%=request.getContextPath()%>/courselist/courselist.do" enctype="multipart/form-data">
				      		<input class="btn courseLightBtn noCourse" type="button" value="取消選課">
				      			<input type="hidden" value="${courseVO.cou_num}" name="cou_num">
				      			<input type="hidden" value="updateStatus" name="action">
				      			<input type="hidden" name="AddOrUpdateStatus" value="0">			      			
				      			<input type="hidden" value="${coulist}" name="coulist">			      			
				      			<input type="hidden" value="${size}" name="size">		
				      			<c:set var="i" value="1"/>	      			
				      		</form>
				      	</c:if>
				      	<c:if test="${(coul.cou_num == courseVO.cou_num) && (coul.pay_status == 2) && (i == 0)}"><c:set var="i" value="1"/><div>已取消選課</div></c:if>
				      	<c:if test="${(coul.cou_num == courseVO.cou_num) && (coul.pay_status == 1) && (i == 0)}"><c:set var="i" value="1"/><div class="payOK">已繳費</div></c:if>
			      	</c:forEach>
			      </c:if>
			      <% if(divColor == 7){divColor = 0;} %>
 	 ...
 	 $(".noCourse").click(function() {
     	...
     	swal({
            ...
            showCancelButton: true,
        	   ...
        	   function (result) {
            if (result) {
                swal("完成!", "課程已取消", "success").then(function(){
                	b.submit();
                });    
            }
 	...
 	$(".signCourse").click(function() {
     	...
     	swal({
            ...
            showCancelButton: true,
        	   ...
        	   function (result) {
            if (result) {
                swal("完成!", "已選課完成", "success").then(function(){
                	b.submit();
                });    
            }
        }, function(dismiss) { // dismiss can be "cancel" | "overlay" | "esc" | "cancel" | "timer"
