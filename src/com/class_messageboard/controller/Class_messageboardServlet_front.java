...
public class Class_messageboardServlet_front extends HttpServlet{
	...
	public void doPost(HttpServletRequest req , HttpServletResponse res) throws ServletException,IOException{
		...
		if("insert".equals(action)) {
			...
			try {
				...
				class_messageboardVO.setGd_id(stVO.getGd_id());
				class_messageboardVO.setT_num(t_num);
				class_messageboardVO.setMsg(msg);
				
				if(!errorMsgs.isEmpty()) {
					req.setAttribute("class_messageboardVO", class_messageboardVO);
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/class_msg/select_page.jsp");
					failureView.forward(req, res);
					return;
				}
				/***************2.開始新增資料***********************/
				Class_messageboardService class_messageboardSvc = new Class_messageboardService();
				class_messageboardVO = class_messageboardSvc.addFontClass_messageboard(cs_num, stVO.getGd_id(), msg);
				/***************3.新增後轉交************************/
				String url = "/front-end/class_msg/select_page.jsp";
//				RequestDispatcher successView = req.getRequestDispatcher(url);
//				successView.forward(req, res);
				res.sendRedirect(req.getContextPath() + "/front-end/class_msg/select_page.jsp");
				return;
				/***************4.處理其他錯誤**********************/
			}catch(Exception e) {
