...
public class class_picsServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)throws ServletException,IOException{
		...
		if("getOne_For_Display".equals(action)) {
			...
			try {
				...
				if(!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/class_pics/search_page.jsp");
					failureView.forward(req, res);
					return;
				}
				
				/**************3.查詢完成後轉交**************************/
				String url = "/back-end/class_pics/search_page.jsp";
		...
		if("getOne_For_Update".equals(action)) {
			...
			try {
				...
				String url ="/back-end/class_pics/update_input_page_2.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
				/*************4.處理其他錯誤****************************/
			}catch(Exception e) {
				errorMsgs.add("無法取得資料" + e.getMessage());
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/class_pics/update_input_page_2.jsp");
				failureView.forward(req, res);
			}
		}
		...
		if("update".equals(action)) {
			...
			try {
				...
				String pic_num = req.getParameter("cs_pic_num");
				
				cs_pic_num = new Integer(pic_num);
				...
				Class_picturesVO class_pictures = class_picturesSvc.getOneClass_pictures(cs_pic_num);
				
				
				class_picturesVO = class_picturesSvc.updateClass_pictures(cs_pic_num, cs_num, ul_date, class_pictures.getPic(), pic_cs);
				/****************3.修改完成後轉交******************/
				req.setAttribute("class_picturesVO", class_picturesVO);
				String url = "/back-end/class_pics/update_input_page.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
				/****************4.處理其他資料********************/
			}catch(Exception e) {
...
//				}
				...
				if(!errorMsgs.isEmpty()) {
					...
					req.setAttribute("class_picturesVO", class_picturesVO );
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/class_pics/select_page.jsp");
					failureView.forward(req, res);
					return;
				}
				/***********2.開始新增資料****************************/
				...
				/***********3.新增完成後轉交**************************/
				String url = "/back-end/class_pics/select_page.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
				/***********4.處理其他錯誤***************************/
			}catch(Exception e) {
